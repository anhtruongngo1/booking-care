    ...
    <div className="h-[300px] w-full bg-white cursor-pointer">
        <div className="mx-[15%] flex flex-col">
            <div className="h-[60px] mb-5 pt-5 ">
                <span className="text-xl font-semibold">{{ $t('home.topDoctor') }}</span>
                <router-link
                    :to="`/doctor`"
            ...
            <Carousel v-bind="settings" :breakpoints="breakpoints">
                <Slide v-for="item in dataDoctor" :key="item.id">
                    <router-link
                        :to="`/doctor/${item.id}`"
                        className="border border-solid border-[#EEEEEE] w-[250px] m-2"
                    >
                        <div
                            className="w-full h-[200px] bg-center bg-cover bg-no-repeat bg-white flex items-center flex-col"
                        >
                            <div className="w-full  h-[120px] mt-2.5 flex  justify-center">
                                <img className="w-[120px] h-[120px] rounded-[50%]" :src="`${item.image}`" />
                            </div>
                            <span className="block text-center mt-5">{{ item.firstName }} {{ item.lastName }}</span>
                        </div>
                    </router-link>
                </Slide>
