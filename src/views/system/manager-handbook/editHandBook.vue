    ...
    <main className="user-redux-container">
        ...
        <div class="pt-6 float-right mr-10">
            <button
                ...
                :disabled="isDisabled"
                class="disabled:opacity-60 py-2 px-4 bg-[#0071BA] font-medium text-white rounded-3xl"
            >
                Sửa đổi
            </button>
        </div>
    ...
    setup() {
        ...
        const handleData = async (id) => {
            ...
            if (res && res.errCode === 0) {
                (title.value = res.blog.title), (content.value = res.blog.content), (thumb.value = res.blog.thumb);
            }
        };
        if (route.params.id) {
